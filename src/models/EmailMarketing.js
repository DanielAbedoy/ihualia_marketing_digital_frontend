...
class EmailMarketing {
  ...
  //NUEVOS DESPUES DE ELIMINAR LOS MODELS DE MAS


  add_boletin = (datos) => {
    return axios.post(`${new URLs().getUrlPrincipal()}/api/boletin/`, datos)
      .then(r => ({ data: r.data, statusText: r.statusText }))
      .catch(e => "error")


  }

  modificar_boletin = (boletin, datos) => {
    return axios.patch(`${new URLs().getUrlPrincipal()}/api/boletin/${boletin}/`, datos)
      .then(r => ({ data: r.data, statusText: r.statusText }))
      .catch(e => "error")
  ...
  send_email = async (email, html, asunto) => {
    return axios.post(`${new URLs().supporserver()}/email/send-email`,
      { email: email, html: html, asunto: asunto },
