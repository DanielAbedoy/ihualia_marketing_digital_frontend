...
class Crear extends Component {
	...
	componentDidMount = () => {
		...
		this.idCuenta = this.context.cuenta.id
		this.getGrupos();
	}
	...
	crearNuevoGrupo = e => {
		...
		new ModeloContactos().crearGrupo(this.state.nombre_grupo_nuevo, this.idCuenta)
			.then(response => {
				if (response !== "Created") {
				...
				<Row>
					<Col xs="12">
						<Card>
                          ...
                          <Col md="2" xs="2">
                            <div onClick={this.crearNuevoGrupo} className="btn-h bg-h-success text-white py-2 ">
									...
									<Col xl="9" lg="11" xs="12" className="mx-auto">
										<Row>
											{this.state.grupos.map((grupo, indx) => {
												return (<GrupoComponent indx={indx} key={indx} grupo={grupo} reload={() => this.getGrupos()} />);
											})}
										</Row>
									</Col>
								</Row>
